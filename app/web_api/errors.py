...
class LoanStartDateOnWeekendError(Exception):
    pass


class LoanEndDateOnWeekendError(Exception):
    pass
...
def validate_loan_inputs(
    ...
    if 0 > annual_margin_in_percent:
        raise IncorrectMarginError(f"'annual_margin_in_percent' has to be greater than or equal to 0.")
    ...
    if start_date.weekday() in [5, 6]:
        raise LoanStartDateOnWeekendError(f"Loan 'start_date' can't be weekend day.")
    if end_date.weekday() in [5, 6]:
        raise LoanStartDateOnWeekendError(f"Loan 'end_date' can't be weekend day.")
